...
def main():
    ...
    parser.add_argument('message', type=str, help='Message to save model as')
    parser.add_argument('--total_timesteps', type=int, default=200_000, help='The total number of samples (env steps) to train on')
    ...
    for _ in tqdm(range(args.repeat)):
        for alg in tqdm(algs):
            ...
            model = alg(
                ...
                # verbose=1,
                # batch_size=512,
                tensorboard_log=f"./logs/{env_id}_{args.message}/",
                ...
                # ),
                # policy_kwargs=policy_kwargs if len(policy_kwargs) else None,
            ...
            eval_callback = EvalCallback(
                ...
                verbose=1,
                best_model_save_path=f'models/{env_id}_{alg.__name__}_{args.message}/',
            )
